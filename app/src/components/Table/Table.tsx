...
type ColumnType =
  ...
  | {key: string; label: string; type: 'icon'; icon: string; onClick: (row: unknown) => void}
  | {key: string; label: string; type: 'custom'; render: (row: unknown) => React.ReactNode}
...
const Table = <T extends Record<string, unknown>>({perPage, columns, elements}: Props<T>) => {
          ...
          {paginated.map((el, idx) => (
            <tr key={idx}>
              {columns.map((col) => {
                ...
                if (col.type === 'custom') {
                  return <td key={col.key}>{col.render(el)}</td>
                }
                return null
              })}
            </tr>
